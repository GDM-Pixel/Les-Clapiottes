    ...
    public function getData(WP_REST_Request $request)
    {
        ...
        switch ($select) {
	        case 'innerlinks':
	            ...
	            if ($rows = $wpdb->get_results("SELECT `ID` FROM ($select_table) as p WHERE (p.post_content LIKE '%$url%' OR p.post_content LIKE '%$url_backslash%' OR p.post_content LIKE '%$url_encoded%')")) {
	                if (!empty($rows)) {
	                    foreach ($rows as $row) {
	        ...
	        case 'keyword':
		        ...
		        //get post inner links
		        $select_table = $wpdb->prepare("SELECT ID, post_content FROM `$wpdb->posts` WHERE `post_status` = %s ORDER BY ID DESC LIMIT %d,%d", 'publish', $start, $limit);
		        if ($rows = $wpdb->get_results("SELECT `ID`, `post_content` as content FROM ($select_table) as p WHERE (p.post_content REGEXP '$regex')")) {
		        ...
		        if ($rows = $wpdb->get_results("SELECT `ID`, `meta_value` as content FROM ($select_table) as p INNER JOIN `$wpdb->postmeta` as pm on p.ID = pm.post_id WHERE pm.meta_value REGEXP '$regex'")) {
			        if (!empty($rows)) {
				        foreach ($rows as $row) {
					        if(untrailingslashit(get_permalink($row->ID)) <> $url) {
						        $row->content = str_replace( '\/', '/', $row->content );
